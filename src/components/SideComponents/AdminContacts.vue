      ...
      <section v-for="(admin, index) in adminInfo" :key="index" class="admin-container">
        ...
        <div class="admins-information">
          <div class="handle-title">
            <h2 data-text="Handle:"><span class="blue-text">{{ admin.handle }}</span></h2>
          </div>
          <div class="organization-title">
            ...
            <h2 data-text="Organization:"><span>{{ admin.organization }}</span></h2>
          </div>
          <div class="name-title">
            <span :class="['icon', admin.publish.organization ? 'eye-true' : 'eye-closed']">
              <component :is="admin.publish.organization ? IconEyeTrue : IconEyeClosed" />
            </span>
            <h2 data-text="Name:"><span>{{ admin.name }}</span></h2>
          </div>
        </div>
      </section>
    </section>
